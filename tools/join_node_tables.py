...
def format_table_row(hostname, merged_data, host_col_width):
    ...
    # CPU count with fallback to API data
    cpu_count = resources.get('cpu_count', es_data.get('cpu', '?'))
    ...
    mem_total = (
        ...
        None  # Use None to indicate unknown value
    )
    ...
    try:
        if mem_total is None:
            ...
            if mem_total_gb == 0:
                mem_total_gb = "?"  # Use ? if conversion results in 0
    except (ValueError, TypeError):
        mem_total_gb = "?"  # Use ? on conversion error
    ...
    try:
        if mem_free is None:
            ...
            if mem_free_gb == 0:
                mem_free_gb = "?"  # Use ? if conversion results in 0
    except (ValueError, TypeError):
        mem_free_gb = "?"  # Use ? on conversion error
    
    # Disk info from ansible facts
    server_disk_total = resources.get('disk_total_gb', "?")
    server_disk_free = resources.get('disk_free_gb', "?")
    
    # Service status - if we have ES API data, we know ES is running
    components = server_data.get('components', {})
    es_component = components.get('elasticsearch', {})
    
    # If we have ES data from the API, consider ES as running regardless of server data
    es_status = 'OK' if (es_data and len(es_data) > 0) or es_component.get('status') == 'running' else 'NO'
    ...
    if mem_total_gb == "?" or mem_free_gb == "?":
        mem_used_gb = "?"
    else:
        mem_used_gb = mem_total_gb - mem_free_gb
        
    if server_disk_total == "?" or server_disk_free == "?":
        server_disk_used = "?"
    elif isinstance(server_disk_total, (int, float)) and isinstance(server_disk_free, (int, float)):
    ...
    # Special handling for memory and disk formatters based on type
    if mem_used_gb == "?" or mem_total_gb == "?":
        mem_format = f"{'?/?'}"
    else:
        mem_format = f"{mem_used_gb}/{mem_total_gb}"
        
    if server_disk_used == "?" or server_disk_total == "?":
        disk_format = f"{'?/?'}"
    else:
        disk_format = f"{server_disk_used}/{server_disk_total}"
...
def main():
    ...
    # Filter for nodes that have Elasticsearch installed or with ES API data
    elasticsearch_nodes = {}
    for hostname, data in node_groups.items():
        ...
        # Include if either:
        # 1. Elasticsearch is installed and running according to server data, OR
        # 2. There is ES API data for this node
        if es_component.get('status') == 'running' or (es_data and len(es_data) > 0):
    ...
    print("  RAM and Disk: Used/Total (GB), ? = Unknown")
